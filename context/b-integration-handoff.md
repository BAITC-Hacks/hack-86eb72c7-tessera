...
После отдельной проверки источников оба ZIP и 12 XLSX побайтово сохранены в `.agent-loop/source-data/` и смонтированы read-only в `/app/local-source-data`; 14/14 SHA-256 совпали. Нормализованных записей нет: текущая БД содержит 0 проектов, 0 импортов, 0 DatasetVersion. Рабочее состояние источников — `preserved_unmapped`; A не должен трактовать mount как завершённый 09a или готовый DatasetVersion. IEK динамика имеет XML-запись 88 043 249 байт; parser cap поднят до 96 MiB, без изменения остальных лимитов. Локальные 24/24 теста импорта и Docker build прошли, но импорт всех 12 книг не запускался.
