...
export function RecommendationTable({ rows, drafts, saved, checkedSuppliers, degraded, approved, onDraftChange, onSave, onReset, onCheck }: Props) {
    ...
    <section aria-labelledby="recommendations-heading" className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex w-full flex-wrap gap-2 sm:w-auto">
          ...
          <label className="text-xs">Срочность
      ...
      {visibleRows.length === 0 ? (
        ...
        return (
          <div key={supplierId} className="rounded-lg border border-border bg-card p-3 sm:p-4">
            ...
            <div tabIndex={0} role="region" aria-label={`Таблица рекомендаций: ${supplierName(supplierId)}`} className="relative mt-3 max-w-full overflow-x-auto focus-visible:outline-2 focus-visible:outline-ring">
            ...
            {supplierRows.filter((row) => expanded.has(row.id)).map((row) => <div key={`details-${row.id}`} id={`details-${row.id}`} className="mt-3"><RecommendationDetails row={row} degraded={degraded} /></div>)}
          </div>
        );
      })}
    </section>
