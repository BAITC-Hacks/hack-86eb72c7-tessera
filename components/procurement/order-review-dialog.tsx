  ...
  return (
    <Dialog open={mode !== null} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent showCloseButton={false} className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>{mode === "approve" ? "Утвердить заказ в демо" : mode === "preview" ? "Экспорт (демо)" : "Сбросить ручные правки?"}</DialogTitle>
          <DialogDescription>{mode === "discard" ? "Ручные изменения текущего расчёта будут потеряны." : "Проверка и предпросмотр работают только в памяти этой страницы."}</DialogDescription>
        </DialogHeader>
        {mode === "discard" ? (
          ...
          <>
            ...
            {mode === "preview" && (
              <>
                ...
                <p className="text-xs text-muted-foreground">Это предпросмотр. Скачивание станет доступно после подключения сохранения заказов.</p>
              </>
            )}
            {mode === "approve" && <p className="rounded-md border border-border p-3 text-sm font-medium">Это демонстрация. Заказ не сохраняется и не отправляется поставщику.</p>}
          </>
        )}
        <DialogFooter>
          <Button type="button" variant="outline" onClick={onClose}>{mode === "preview" ? "Закрыть" : "Отмена"}</Button>
          {mode !== "preview" && <Button type="button" onClick={onConfirm}>{mode === "approve" ? "Подтвердить в демо" : "Сбросить и продолжить"}</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
